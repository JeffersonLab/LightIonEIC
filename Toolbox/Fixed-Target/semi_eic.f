...
c  fsgen 100K event generated.....fora moment
     ...
     &       ACTION='READ')
c        OPEN(unit=99,file='./gn_ed_eic_copy.txt', status='old',
c     &       ACTION='READ')

         DO ll=1,99999
c         DO ll=1,99
          read (99,*) h1,h2,h(1),h(2),h(3)
     ...
     &          ,sp(7),sp(8),sp(9),sp(10),sp(11)
           read (99,*) zz(1),zz(2),zz(3),zz(4),zz(5),zz(6)
     &          ,zz(7),zz(8),zz(9),zz(10),zz(11)
...
c invariant mass square definition
     ...
     &       +2.0*(dm-E_r)*q0 +2.0*p_r*qv_mag*cos(xnthetap_r)-q2
        ...
        phi_qp = phi_h_cm
c$$$
c$$$        print *,beta,theta_qp,the_hd_cm,phi_hd_cm,pfscm

c Misak's definition
c        wn2 =  dm**2 - 2.0*E_r*dm    + pm**2 +  
c     &	       2.0*q0*(dm-E_r)    + 2.0*qv*p_r_z - Q2
...
c initialization 0: calculation, 1: PWIA
        ...
        alpha_r = 2*(E_r - p_rz)/(ed - pd) 

        wn_BONUS2 = ((dm-E_r)**2-(p_r)**2) + 2*dm*q0*(2-alpha_r)-q2
        wn_BONUS = sqrt(wn_BONUS2)
...
c t_min definition from M. Guidal
         E1_cm = (wn**2-q2-dm**2)/(2*wn)
         P1_cm = sqrt(E1_cm**2 + q2)

         E2_cm = (wn**2+q2+dm**2)/(2*wn)
         P2_cm = sqrt(E2_cm**2 - dm**2)

c       part3 (E3, P3) information is meaningless
         E3_cm = (wn**2-dm**2)/(2*wn)
         P3_cm = sqrt(E3_cm**2)

c       part4 (E4, P4) even not precise due to m3 information ignored
         E4_cm = (wn**2+pm**2)/(2*wn)
         P4_cm = sqrt(E4_cm**2 - pm**2)

         E_cm = E1_cm - E3_cm
         P_cm = P1_cm - P3_cm
         E_cm2= E2_cm - E4_cm
         P_cm2= P2_cm - P4_cm
c not trust these tmin values
         xMG_tmin = -1*(E_cm**2 - P_cm**2)
         yMG_tmin = -1*(E_cm2**2-P_cm2**2)

c  recoil nucleon momentum, energy  in the intermediate state
c         p_r1 = sqrt((p_rx-qx)**2 + (p_ry-qy)**2 + (p_rz-qz)**2)
         p_r1 = sqrt((p_rx)**2 + (p_ry)**2 + (p_rz)**2)
         E_r1 = sqrt(pm**2+p_r1**2)

cc         - u_chan - t_chn + W**2 =  -Q2 + 2*mprot**2 + mpion**2
         qv2pr = (qx)*(p_rx)+(qy)*(p_ry)+(qz)*(p_rz)
         cosThetaCM_u = 1/(P1_cm*P4_cm)*(qv2pr)
         u_chn =  -((E1_cm-E4_cm)**2-(P1_cm-P4_cm)**2
     &        -2*P1_cm*P4_cm*(1-cosThetaCM_u))
         u_max =  -((E1_cm-E4_cm)**2-(P1_cm-P4_cm)**2-4*P1_cm*P4_cm)
         u_min =  -((E1_cm-E4_cm)**2-(P1_cm-P4_cm)**2)

         pr2prtemp = (p_rx)*(p_rx)+(p_ry)*(p_ry)+(p_rz)*(p_rz)

c     Misak's definition
c	pr1pr = p_r1_x*p_r_x +  p_r1_y*p_r_y +  p_r1_z*p_r_z 
c	t    = 2.0*pm**2 - 2.0*E_r*E_r1 + 2.0*pr1pr
c       tprime = t - M_N**2 = 2*p_r**2 [true]

c         t_chn_MK = 2.0*pm**2 - 2.0*E_r*E_r1 + 2.0*pr2prtemp
...
cccccc    F2D with free nucleon -> ./hbook_f2df/
         ...
         alpha_r0=1
         p_rt0 =0
        call edenx(se,sq,q2,q0,ktr,alpha_r0,p_rt0,x,s,si0,
     &       Fd_L,Fd_T,Fd_TL,Fd_TT,f2d0,f1eff,f2eff,sun,icase,lc,lbj)
...
c        print *,si,crs,crs1
        ...
        if(sqrt(se).ge.0.0)then
           if(si.ge.0 .and. xmx2.ge.0 .and. tprime_KJ.ge.0)then
              ...
              write(iu,10) x,q2,p_rt,alpha_r,si,p_x2,e_x,xmx2,wn,
     &             tprime_KJ,t_chn_MK,theta_qp,crs1,crs,
     &             p_rx,p_ry,p_rz,yy
