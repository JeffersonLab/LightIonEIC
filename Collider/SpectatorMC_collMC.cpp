...
int mainx(){
	...
	for (int iEvt=0; iEvt<NEvts; iEvt++) {
		...
		//		if(invts.alphaS>0.97&&invts.alphaS<1.03){
			...
			// var[]=array, var()=func, both have same value return
			...
			//  OUTPUT FOR GEMC 
			//
			OUT << setiosflags(ios::left)  << setiosflags(ios::fixed)  <<"                 "  << MEvts << " \t " << NumPtls << " \t " << setprecision(8)<< scientific  << invts.xBj << " \t " << invts.Q2  << " \t " << invts.s_e  << " \t " << invts.s_q << " \t" << invts.alphaS << " \t" << invts.pPerpS << " \t"  << invts.tPrime  << " \t"  <<  invts.Jacob << " \t"  <<  eBeamPol <<  " \t"  <<  DBeamPol << endl;
       ...
       //       OUT << setiosflags(ios::left) << setiosflags(ios::fixed) << setprecision(8) << "\t" << "1" << " \t " << dum_particle_id << " \t " << "0" <<  " \t "<< scientific << pXx_Lab << " \t " << pXy_Lab << " \t " << pXz_Lab << " \t " << EXE_Lab << " \t " << dummass << " \t " << vXx_Lab  << " \t " << vXy_Lab << " \t " << vXz_Lab << endl; 

			//
			//  OUTPUT FOR ANALYSIS
			//
       // 			OUT << setiosflags(ios::left)  << setiosflags(ios::fixed)  <<"                 "  << MEvts << " \t " << NumPtls << " \t " << setprecision(8)<< scientific  << invts.xBj << " \t " << invts.Q2  << " \t " << invts.s_e  << " \t " << invts.s_q << " \t" << invts.alphaS << " \t" << invts.pPerpS << " \t"  << invts.tPrime  << " \t"  <<  invts.Jacob  << endl;
       // OUT << setiosflags(ios::left) << setiosflags(ios::fixed) << setprecision(6) << "\t" << "1" << " \t " << e_particle_id << " \t " << "0" <<  " \t "<< scientific <<pex_Res << " \t " << pey_Res << " \t " << pez_Res << " \t " << EeE_Res << " \t " << emass << " \t " << vex_Res  << " \t " << vey_Res << " \t " << vez_Res << endl; 
       // OUT << setiosflags(ios::left) << setiosflags(ios::fixed) << setprecision(6) << "\t" << "1" << " \t " << sp_particle_id << " \t " << "0" <<  " \t "<< scientific<< psx_Res << " \t " << psy_Res << " \t " << psz_Res << " \t " << EsE_Res << " \t " << spmass << " \t " << vsx_Res  << " \t " << vsy_Res << " \t " << vsz_Res << endl; 
       // OUT << setiosflags(ios::left) << setiosflags(ios::fixed) << setprecision(6) << "\t" << "1" << " \t " << dum_particle_id << " \t " << "0" <<  " \t "<< scientific << pXx_Res << " \t " << pXy_Res << " \t " << pXz_Res << " \t " << EXE_Res << " \t " << dummass << " \t " << vXx_Res  << " \t " << vXy_Res << " \t " << vXz_Res << endl; 

		    }

// p_n = (P_D - p_S)  as a four-vector, then compute  p_n^2  = t
// compare this event-by-event with
// M_D^2 + M^2 - 2*P_D \cdot p_S = t
	 ...
	 if(abs(invts.alphaS-1)<0.02){
	     htS->Fill(invts.tPrime,1.);
	 }
	 //	invts.tPrime     = 2.*pSpectator_Vertex.Dot(PIncident_Vertex) - MIon*MIon;
