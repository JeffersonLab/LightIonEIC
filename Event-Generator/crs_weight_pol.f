...
C     polarization is taken into account (only C.W. code)
        double precision :: FSIG1,FSIG2,ASYM_LL
        double precision :: YY, EPSout, Depol
...
c     selection of model (M. Sargsian): 1(not available),  (C. Weiss): 2
        imodel = 2
c     Call F2 parameterization  (C.Weiss) : 2,  (P.Jimenez-Delgad): 3
        IPN =2
...
c     ---------------------------------------------------
C
c      POLARIZATION CASE IS ONLY FOR MODEL==2 
C
c     ---------------------------------------------------
...
c        alpha_r = 2*(E_r - p_rz)/(ed - E_ibeam) 
        alpha_r = xinv_alpha 
...
c SHOULD BE SAME FORMAT AS MODEL==2
     ...
     &             crs0,crs1,xebeam_pol,xjacobian,crs2,
     &             xinv_tprim,t_chn_MK,xibeam_pol,xinv_pRTs,xinv_pRT0,
     &             p_rx,p_ry,p_rz,crs0
...
* ------------- (IPN for Weiss code (1):proton, (2):neutron )
           ...
           CALL TAGFN(f2nn,dble(xinv_xbj),dble(xinv_q2),2)
...
C     
         IPN =2
         CALL TAGXP(FSIG1,YY, EPSout, Depol, 
     &        dble(xinv_se),dble(xinv_xbj),dble(xinv_q2),
     &        dble(xinv_alpha),dble(xinv_pRT),IPN, 1)
         CALL TAGXP(FSIG2,YY, EPSout, Depol, 
     &        dble(xinv_se),dble(xinv_xbj),dble(xinv_q2),
     &        dble(xinv_alpha),dble(xinv_pRT),IPN, -1) 
         CALL TAGFD(F2D,FLD,
...
C 
         ...
         IF(sqrt(xinv_se).lt.0.0)RETURN

c     double check (not actually using calculation)
         xjacobian = xinv_jacob
c     ---------------------------------------------

         CALL TPMIN(xinv_tpmin,(xinv_alpha),1)

c     Notation noted from C.Weiss Codes
...
C                  -1  CROSS SECTION SIGMA(+-) = SIGMA(-+)
         IF(FSIG1.le.0.OR.FSIG2.le.0)RETURN
         ASYM_LL = (FSIG1-FSIG2)/(FSIG1+FSIG2)
         asym0 = ASYM_LL
c     FSIG1 : hel=+1  , FSIG2 : hel=-1  
         crs1 = FSIG1
         crs2 = FSIG2
...
c     xinv_se,xinv_sq,xinv_q2,q0,ktr,xinv_alpha,xinv_pRT,
              write(iu,101) nptl,
     &  dble(xinv_xbj),dble(xinv_q2),dble(xinv_se),dble(xebeam_pol),
     &  dble(xinv_alpha),dble(xinv_pRT),dble(xinv_tprim),dble(FSIG1),
     &  dble(FSIG2)
              write(iu,11) egen,echarge,pid,pe_x,pe_y,pe_z,pe_E,pe_M,
     &             ve_x,ve_y,ve_z
              write(iu,11) sgen,scharge,pid1,p_rx,p_ry,p_rz,E_r,pm,
...
c
         write(iw,10) xinv_xbj,xinv_q2,xinv_pRT,xinv_alpha,
     &           YY,EPSout,xebeam_pol,xjacobian,Depol,
     &           xinv_tprim,t_chn_MK,xibeam_pol,FSIG1,FSIG2,
     &           p_rx,p_ry,p_rz,asym0    
...
C###############################################################################
         ...
         ENDIF
C     This 'ENDIF' is for model2 (C.W.)
