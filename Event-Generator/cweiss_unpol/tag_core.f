...
C
C     CORE ROUTINES:
C     TAGPD  -- DEUTERON TAGGED CROSS SECTION, POLARIZED
C     TAGGD  -- DEUTERON TAGGED STRUCTURE FUNCTIONS, POLARIZED
C     TAGCD  -- DEUTERON TAGGED CROSS SECTION, UNPOLARIZED
C     TAGFD  -- DEUTERON TAGGED STRUCTURE FUNCTIONS, UNPOLARIZED
C     TAGSD  -- DEUTERON SPECTRAL FUNCTION
C     TAGVER -- VERSION NUMBER FOR APPLICATIONS
C
C     USER-DEFINED ROUTINES CALLED:
C     TAGG1N -- NUCLEON STRUCTURE FUNCTION G1N, POLARIZED
C     TAGFLN -- NUCLEON STRUCTURE FUNCTION FLN, UNPOLARIZED
C     TAGF2N -- NUCLEON STRUCTURE FUNCTION F2N, UNPOLARIZED
C     TAGWD  -- DEUTERON NON-RELATIVISTIC WAVE FUNCTION
C
C     ADDITIONAL USER-DEFINED ROUTINES (OPTIONAL):
C     TAGRD  -- RESIDUE OF SPECTRAL FUNCTION AT TPRIME = 0
C
C     REVISION HISTORY:
C     V1  13MAY14   UNPOLARIZED CROSS SECTION MODEL
C     V2  30JUN14   ADDED POLARIZED MODEL (TAGXP, TAGGN - RENAMED)
C     V3  31OCT14   CHANGED ROUTINE NAMING SCHEME
C                   CHANGED SOME VARIABLE NAMES (SD, AK) TO NOTATION
C                      USED IN TECHNICAL NOTE
C                   ADDED LONGITUDINAL STRUCTURE FUNCTION FLN
C                   ADDED VERSION NUMBER ROUTINE TAGVER
C
C---------------------------------------------------------------------
C
*DECK TAGPD
      SUBROUTINE TAGPD(FSIG, Y, EPS, D1, SED, X, QQ, ALR, PRT, 
     *   IPN, IPOL)
C
...
C     PRT   RECOIL MOMENTUM, TRANSVERSE
C
...
C
      CALL TAGFD(F2D, FLD, X, QQ, ALR, PRT, IPN)
      CALL TAGGD(G1D,      X, QQ, ALR, PRT, IPN)
C
C     ...CROSS SECTION
C
      FSIG = FACD*(F2D/XD - IPOL*2*D1*G1D)
C
      END
C
C---------------------------------------------------------------------
...
C     ...SPECTRAL FUNCTION
C
      CALL TAGSD(SD, ALR, PRT)
C
C     ...STRUCTURE FUNCTION
C
      XEFF = X/(2 - ALR)
C
      CALL TAGG1N(G1N, XEFF, QQ, IPN)
C
      G1D = 2.D0/(2 - ALR) *SD *G1N
C
      END
C
C---------------------------------------------------------------------
C
*DECK TAGCD
      SUBROUTINE TAGCD(FSIG, SED, X, QQ, ALR, PRT, IPN)
C
C     DEUTERON TAGGED ELECTROPRODUCTION CROSS SECTION
C
C     FSIG  TAGGED ELECTROPRODUCTION CROSS SECTION
...
C
*DECK TAGFD
      SUBROUTINE TAGFD(F2D, FLD, X, QQ, ALR, PRT, IPN)
C
C     DEUTERON TAGGED STRUCTURE FUNCTION IN IMPULSE APPROXIMATION
C
C     F2D   TAGGED DEUTERON STRUCTURE FUNCTION F2D
C     FLD   TAGGED DEUTERON STRUCTURE FUNCTION FLD
C
C     X     NUCLEON SCALING VARIABLE X = 2*XD
C     QQ    4-MOMENTUM TRANSFER Q**2
C     ALR   RECOIL MOMENTUM, LC FRACTION 
...
C
      XEFF = X/(2 - ALR)
C
      CALL TAGF2N(F2N, XEFF, QQ, IPN)
      CALL TAGFLN(FLN, XEFF, QQ, IPN)
C
      F2D = SD*F2N
      FLD = SD*FLN
C
...
C
*DECK TAGSD
      SUBROUTINE TAGSD(SD, ALR, PRT)
C
C     DEUTERON LIGHT-CONE SPECTRAL FUNCTION
C     NORMALIZED SUCH THAT  INTEGRAL (D ALR/ALR) (D**2 PRT) SD = 1
C
C     ALR   RECOIL MOMENTUM, LC FRACTION 
C     PRT   RECOIL MOMENTUM, TRANSVERSE
C
...
C
      AK = SQRT((PRT**2 + UN**2)/ALR/(2 - ALR) - UN**2)
C
C     ...SPECTRAL FUNCTION
C
      CALL TAGWD(PSI, AK)
C
      SD = SQRT(AK**2 + UN**2) *PSI**2 /(2 - ALR)
C
      END
C
C---------------------------------------------------------------------
C
*DECK TAGVER
      SUBROUTINE TAGVER(VER)
C
C     ...PROVIDE VERSION NUMBER FOR APPLICATIONS
C
      CHARACTER*(*) VER
C
      VER = 'V3.0'
